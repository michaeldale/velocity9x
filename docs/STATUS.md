...
| Software Direct3D | Opt-in in the S3, ATI and VBE packages through the settings page or `Direct3D=2` | Same defaults; exact incremental edges, plus an exact divide-by-255, a single colour clamp and per-span format/compare dispatch | [Edge timings](decisions/2026-09-07-software-rasterizer-edge-stepping.md) about 2.2x for the synthetic small-triangle scene, then [scalar fixes](decisions/2026-09-10-rasterizer-scalar-fixes.md) 1.45x on point-sampled modulate and 1.04x to 1.20x on the other scenes; pixel hashes unchanged; no physical or game-speed claim |
...
| Make software Direct3D practical | Edge stepping, the 86Box RAM/aperture benchmark and scalar fixes 2 to 4 are done; next are the plan's texel-unit coordinates and cheaper bilinear, then mixed texture/Z residency and a named-game gate | [Edge stepping](decisions/2026-09-07-software-rasterizer-edge-stepping.md) and [scalar fixes](decisions/2026-09-10-rasterizer-scalar-fixes.md), [remaining scalar plan](plans/software-rasterizer-scalar-fixes.md); every gain shrinks on the VRAM target, and physical timings are still needed before residency policy, hybrid assistance or [SMP workers](plans/software-d3d-smp-workers.md) |
