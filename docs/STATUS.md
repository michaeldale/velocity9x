...
| Matrox Millennium (MGA-2064W) | The BIOS advertises 20 linear-framebuffer modes at its BAR1 base, the kit has set `0117h` and restored it, the aperture round-trips 32-bit accesses once CRTCEXT3 bit 7 is set, and this card measures 8 MiB. A guarded candidate carries the chip | [The candidate](decisions/2026-09-10-the-2064w-is-drivable-by-the-vbe-path.md) and [the aperture and memory](decisions/2026-09-11-the-2064w-aperture-opens-with-mgamode.md); Windows has still set no mode and drawn no pixel, nothing in `src/` sets CRTCEXT3 bit 7 yet, and 8 MiB is this card rather than the part, which shipped in several memory sizes |
