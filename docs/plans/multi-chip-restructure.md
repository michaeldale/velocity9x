...
The runtime dual dispatch (`v9x_trio_engine_ready() ? v9x_trio_copy : v9x_virge_copy` at `:1274`, fills at `:1482`, drains at `:1028`, blt status at `:1542`) collapses into one `const struct v9x_engine32_ops *` vtable: `ready / validate_status / wait_idle / fill / copy / set_display_start / in_vblank / recover / build_caps`.
...
7. **ddhal.c split.** Mechanical extraction along the seams listed above; vtable resolved lazily. Gate: Ironfield FPS unchanged, BLT_ENGINE trace counters nonzero on both S3 targets, Hellbender D3D pass on ViRGE, timeout-injection recovery (fifo_timeouts/reset_count) matches baseline.
8. **S3 family merge** (sync point between source and build tracks). Add `packaging\families\s3\family.psd1` covering both chips; enable per-object and dispatch-symbol audits. Gate: `run-vm-mode-matrix.ps1 -Family s3` passes on **both** 86Box profiles (ViRGE and Trio). Then retire the `s3-virge`/`s3-trio64` manifests, legacy switches, `LegacyOutputName`, and the checked-in INF. Byte-comparability intentionally ends here.
9. **VBE tier-0 end-to-end.** Build the `generic/vbe` family package. Validate on the Matrox 86Box profile first (its path is already VBE-LFB + one pitch hook, nearest to tier-0), then on a chip with no backend. Host tests for the pure parsers (4F00/4F01 result validation, mode-whitelist intersection). **Known limit:** the ViRGE/DX BIOS ignores the generic LFB bit 0x4000 (`runtime.asm:432-436`) — tier-0 is not expected to work there. Automated testing: QEMU std-vga controller variant (`Vm.Controller` manifest key, per PLAN.md §4.3) or an interim forced Have-Disk smoke install on the 86Box S3 VM. Families with no emulator (`Vm.Emulator = 'none'`, e.g. Intel GMA) make the VM runner refuse with an explicit real-hardware-only error.
...
11. **Host tests + docs.** `build-host.ps1` generates `build\host\v9x_family_matrix.h` from the manifests; new `tests\host\test_family_matrix.c` asserts: every declared PCI ID resolves via the registry to the correct family backend, different families resolve to different ops, undeclared IDs return null, per-chip caps ⊆ family caps mask, the VBE backend advertises no accel caps, and `validate_mode` accepts exactly the modes the generated INF advertises (first-ever INF/driver mode-agreement check). Rewrite `docs\BUILDING.md`; new `docs\specifications\family-manifest.md`; decision record for per-family packaging; update VM docs with per-family profiles/ports.
