...
if (-not $BuildId) { $BuildId = Get-V9xBuildId -RepoRoot $repoRoot -Fallback "intel-survey-local" }
...
try {
    & $compiler "-bt=dos" "-ms" "-zq" "-wx" `
        ...
        (Join-Path $repoRoot "tools\diag\intel_survey_dos.c")
    if ($LASTEXITCODE -ne 0) { throw "Failed to build the DOS Intel GMA survey." }
...
foreach ($marker in @($BuildId, "Velocity9x Intel GMA survey", "query-only")) {
    if (-not $text.Contains($marker)) { throw "Intel GMA survey lacks marker $marker" }
}
