/*
 * Back-face culling. See d3d_cull.h for why it is decided here rather than
 * by the hardware.
 *
 ...
 * infinity. */
#define V9X_D3D_CULL_EXPONENT_MASK 0x7f800000ul
                          ...
                          float cx, float cy)
{
    ...
    if (mode != V9X_D3DCULL_CW && mode != V9X_D3DCULL_CCW) {
        return 0;
    }
     ...
     * can be trusted to say so under Open Watcom: tests\host\test_d3d_cull.c
     ...
     */
    {
        ...
        stored.value = area;
        if ((stored.bits & V9X_D3D_CULL_EXPONENT_MASK) ==
                V9X_D3D_CULL_EXPONENT_MASK) {
            return 0;
        }
    }
    /* Strict both ways: zero area covers no pixel, so it is drawn. */
    if (mode == V9X_D3DCULL_CW) {
...
{
    if (mode == V9X_D3DCULL_CW && claims_cw != 0) {
        return V9X_D3DCULL_CW;
    }
    if (mode == V9X_D3DCULL_CCW && claims_ccw != 0) {
        return V9X_D3DCULL_CCW;
    }
    return V9X_D3DCULL_NONE;
}
